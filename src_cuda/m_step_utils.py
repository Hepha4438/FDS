...
def train_global_model(
    ...
    """
    Train global transformation model on aggregated data using Adam Optimizer.
    """
    model.train()
    ...
    with torch.no_grad():
        ...
        logging.info(f"    Source feature norms: mean={source_norms.mean():.4f}, std={source_norms.std():.4f}")
        logging.info(f"    Target feature norms: mean={target_norms.mean():.4f}, std={target_norms.std():.4f}")

    # =====================================================================
    # TỐI ƯU HÓA M-STEP BẰNG ADAM OPTIMIZER (NGUYÊN BẢN)
    # =====================================================================
    logging.info("  [M-step] Chạy Adam Optimizer truyền thống...")
