...
def align_features_fgw(
    ...
    # Training parameters
    n_iters: int = 5,  # [Flash-USHER] Giảm từ 30 xuống 5 vòng (thường sẽ dừng ở vòng 3)
    early_stop_threshold: float = 0.5, # [Flash-USHER] Ngưỡng dừng sớm (0.5% xê dịch)
    ...
    global_expert_indices_a = torch.zeros(n_a, dtype=torch.long, device=device_t)
    for b_idx, (src_idx, tgt_idx) in enumerate(batches):
        if src_idx is not None:
            global_expert_indices_a[torch.from_numpy(src_idx).long()] = b_idx

    # Initialize shared model and optimizer (across all batches)
    from model_utils import FeatureTransformMoE
    model = FeatureTransformMoE(input_dim=d_a, output_dim=d_b, num_experts=len(batches)).to(device_t)
    ...
    def apply_model_with_scaling(features_in: torch.Tensor, expert_indices: torch.Tensor) -> torch.Tensor:
        ...
        if global_feature_mean is not None and global_feature_std is not None:
            ...
            # Apply model [Flash-USHER: Truyền ID Cụm vào MoE]
            output_std = model(features_std, expert_indices)
            ...
            return model(features_in, expert_indices)
    ...
    for it in tqdm(range(n_iters), desc="E-M Alignment"):
        ...
        if it == 0:
            ...
            if m_step_method == 'transfer':
                ...
                with torch.no_grad():
                    features_source_base = apply_model_with_scaling(features_a, global_expert_indices_a)

        # Chuẩn hóa MỘT LẦN DUY NHẤT cho toàn bộ dữ liệu
        if metric == 'cosine':
        ...
        def process_cluster(b_idx, src_idx, tgt_idx):
            ...
            with ctx:
                ...
                # Gọi Batch Solver (Không cần tính Normalize ở đây nữa)
        ...
        # ====================================================================
        # [Flash-USHER OOM Fix] 2. Giới hạn luồng (Throttle Workers)
        # ====================================================================
        # if device_t.type == 'mps':
        #     max_workers = 2  # Kiến trúc Unified Memory của Mac dễ đầy, chạy 2 luồng an toàn
        # else:
        #     max_workers = min(len(batches), 8) # CUDA xử lý bộ nhớ riêng tốt hơn
        max_workers = min(len(batches), 11)
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                logging.info(f"  Batch {b_idx+1}/{len(batches)}: Transport computed (GPU Parallel)")

        if device_t.type == 'cuda':
            torch.cuda.synchronize(device=device_t)
        elif device_t.type == 'mps':
            torch.mps.synchronize()
            torch.mps.empty_cache()
        
        # Track convergence by comparing mappings with previous iteration
        max_pct_changed = 0.0  # Biến theo dõi cụm có sự xê dịch lớn nhất
        ...
        for batch_idx, result in enumerate(batch_results):
            ...
            if it > 0 and batch_idx in prev_iter_mappings:
                ...
                # Cập nhật mức độ xê dịch lớn nhất
                if pct_changed > max_pct_changed:
                    max_pct_changed = pct_changed
                ...
                logging.info(f"  Convergence: Cluster {batch_idx}: {n_changed}/{n_total} cells changed ({pct_changed:.1f}%)")

            # Store current mapping for next iteration
            prev_iter_mappings[batch_idx] = mapping_curr

        # ============================================================
        # [Flash-USHER] LOGIC EARLY FREEZING / EARLY STOPPING
        # ============================================================
        if it > 0 and max_pct_changed < early_stop_threshold:
            logging.info(f"=== [Flash-USHER] HỘI TỤ CHỚP NHOÁNG KHÍCH HOẠT ===")
            logging.info(f"Cấu trúc vi mô đã đóng băng (Tối đa chỉ {max_pct_changed:.2f}% tế bào xê dịch).")
            logging.info(f"Dừng vòng lặp EM sớm tại Iteration {it + 1}!")
            break  # Kích hoạt thoát vòng lặp EM ngay lập tức
        ...
        if m_step_method == 'global':
            ...
            source_agg, target_agg, expert_idx_agg, agg_stats = aggregate_training_data_from_batches(
        ...
        if debug_plots_path:
            ...
            if m_step_method == 'transfer':
                ...
                with torch.no_grad():
                    ...
                    a_hat_global = apply_model_with_scaling(features_a, global_expert_indices_a)
    ...
    # Tái tạo T_full trên CPU thay vì GPU để tránh nổ RAM
    logging.info("Reconstructing full transport matrix for output on CPU...")
    # [Flash-USHER Mac Fix] Đổi device_t thành 'cpu'
    T_full = torch.zeros(n_a, n_b_orig, device='cpu', dtype=torch.float32)

    for result in batch_results:
        # Kéo T từ GPU về CPU ngay lập tức
        T = result['T'].cpu() 
        source_indices = result['source_indices']
        target_indices = result['target_indices']

        if use_stratified_pairing:
            source_idx_tensor = torch.from_numpy(source_indices).long()
            target_idx_tensor = torch.from_numpy(target_indices).long()
